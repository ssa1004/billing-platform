...
→ 16 byte prefix 가 정확성과 비용의 sweet spot.

### 왜 full body 비교가 아닌가

같은 의도로 일부 시스템은 first request body 자체를 Redis 에 저장 후 byte-by-byte 비교.
...
본 use-case 는 공격이 아닌 client bug 검출이라 timing attack 위험은 낮지만, 표준 보안
...
같은 키로 동시 두 호출이 와서 둘 다 fingerprint 가 비어 있다고 봤을 때:

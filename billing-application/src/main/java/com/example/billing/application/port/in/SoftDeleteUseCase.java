 ...
 * 사고는 이 트랜잭션 경계로 차단합니다.</p>
 *
 * <p><b>이미 삭제된 row 재호출</b>: 멱등 — 이미 deleted_at 이 set 인 row 에 다시 호출하면
 * UPDATE 가 0행을 영향받아 false 반환. 호출자는 boolean 으로 "처음 삭제" 여부를 안 후 audit 를
 * 한 번만 발행. 두 번 발행되면 timeline 이 어지러워지므로.</p>
 */
public interface SoftDeleteUseCase {
    ...
    /** Refund 논리 삭제. */
    boolean softDeleteRefund(RefundId refundId, AuditActor actor, String reason);
}

 ...
 *
 * [saveIfAbsent] 는 eventId UNIQUE 제약 위반 시 false 반환 (중복 무시). DB 단에서
 * 멱등성을 보장하므로 application 측 lock 불필요.
 */
interface UsageEventRepository {

    /** @return true = 새로 저장됨, false = 이미 존재 (멱등성으로 무시) */
    fun saveIfAbsent(event: UsageEvent): Boolean

    fun existsById(eventId: UUID): Boolean

    fun findInRange(fromInclusive: Instant, toExclusive: Instant, limit: Int): List<UsageEvent>
}

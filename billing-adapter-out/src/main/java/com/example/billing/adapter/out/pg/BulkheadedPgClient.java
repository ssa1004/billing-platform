 ...
 * (wallet 조회, invoice 발급, audit 조회 등) 까지 stall* 됩니다. Netflix Hystrix 가 풀던
 * 고전적 cascade failure (한 종속이 느려져 caller 까지 무너지는 현상) — 카카오 / Line / 토스
 * 페이먼츠 모두 도메인별 ThreadPool 격리로 대응.</p>
 *
 * <p><b>흐름</b>:</p>
 * <pre>
 ...
 * worker 가 막혀도 caller 자체는 풀림. 따라서 도메인 cascade 에 강함.</p>
 *
 * <p><b>호출 인터페이스는 동기 그대로 유지</b>: 가상 스레드 (Java 21 Virtual Threads) 환경에선
 * caller 가 잠시 wait 해도 OS thread 는 안 잡힘 — 동기 인터페이스 + bulkhead 격리만으로 충분히
 * cascade 차단. CompletableFuture 를 application service 까지 끌고 가는 큰 변경 없이 효과만
 * 가져옵니다. ADR-0026 참고.</p>
 *

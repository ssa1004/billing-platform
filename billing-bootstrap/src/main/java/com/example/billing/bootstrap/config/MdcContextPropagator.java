...
/**
 * Resilience4j {@link ContextPropagator} 구현 — caller thread 의 SLF4J {@link MDC} 를
 * ThreadPoolBulkhead worker thread 로 전파.
 *
 * <p><b>왜 필요</b>: ADR-0026 의 ThreadPoolBulkhead 는 외부 호출 (PG / webhook / audit-export)
 * 을 *별도 worker pool* 에서 실행. MDC 는 {@link ThreadLocal} 기반이라 worker thread 에는
 * 자동으로 안 따라갑니다. 결과:
 * <ul>
 *   <li>caller thread (servlet / 가상 스레드) 의 traceId / spanId / requestId / customerId 가
 *       worker 에 비어있음.</li>
 *   <li>{@code RestClientPgClient}, {@code RestClientWebhookHttpClient} 가 찍는 *PG 호출 로그*
 *       가 분산 추적 join 에서 빠짐 — Grafana 에서 한 결제 trace 를 따라가다가 PG 호출 레인이
 *       끊기는 현상.</li>
 *   <li>같은 이유로 Sentry / Datadog 의 error tagging 도 끊김.</li>
 * </ul>
 *
 * <p><b>Resilience4j 의 ContextPropagator 명세</b>:
 * <ul>
 *   <li>{@link #retrieve()} — caller thread 에서 호출. 전파할 값을 추출 (snapshot).</li>
 *   <li>{@link #copy()} — worker thread 진입 시 호출. snapshot 으로 worker 의 ThreadLocal 셋업.</li>
 *   <li>{@link #clear()} — worker 작업 끝나면 호출. ThreadLocal 정리 (memory leak 방지).</li>
 * </ul>
 * 세 단계가 분리되어 있어 *thread reuse* (worker 가 풀에 반환 후 다음 작업에 재사용) 시
 * 이전 작업의 MDC 가 남아 *오염* 되는 문제를 막을 수 있습니다.
 *
 * <p><b>왜 전체 MDC 를 통째로 전파하는가</b>: 특정 키만 골라 (whitelist) 옮기는 방식은 빠르지만
 ...
 *
 * <p><b>주의 — {@code Optional<Map>} 라는 wrapping</b>: Resilience4j 의 인터페이스는
 * {@code Supplier<Optional<T>>}. caller MDC 가 비어있으면 ({@link MDC#getCopyOfContextMap()} 가
 * null 반환) {@link Optional#empty()} 를 돌려줘야 합니다 — 비어있는 Map 을 그대로 넘기면 worker
 * 가 빈 MDC 로 *덮어쓰기* 해버려 (clear 단계에서) thread reuse 에 노이즈.
 *
 * <p><b>cleanup 의 의미</b>: worker 작업 끝나면 *원래 worker 가 갖고 있던 MDC* 로 복원 — 즉
...
public final class MdcContextPropagator implements ContextPropagator<Map<String, String>> {
    ...
    public Supplier<Optional<Map<String, String>>> retrieve() {
        return () -> {
            ...
            if (ctx == null || ctx.isEmpty()) return Optional.empty();
            // 불변 복사 — caller 가 이후에 MDC 를 변경해도 worker 가 보는 snapshot 은 고정.
            return Optional.of(Map.copyOf(ctx));
        };
    }

    @Override
    public Consumer<Optional<Map<String, String>>> copy() {
        return optCtx -> optCtx.ifPresent(ctx -> {

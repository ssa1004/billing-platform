...
/**
 * Redis 가 비활성인 dev / test 환경의 fallback [AdminRateLimiter] — 항상 허용.
 *
 * [RedisAdminRateLimiter] 가 등록되지 않은 경우에만 활성 (`@ConditionalOnMissingBean`).
 * 운영에서는 Redis 가 켜져 있어야 한다 — 이 fallback 으로 떨어지면 admin endpoint 의 보호가
 * 사라지므로 boot 로그에 명시.
 */
@Configuration
class NoOpAdminRateLimiterConfig {

    @Bean
    @ConditionalOnMissingBean(AdminRateLimiter::class)
    fun adminRateLimiter(): AdminRateLimiter = NoOp()

    internal class NoOp : AdminRateLimiter {
        override fun tryConsume(scope: String, callerKey: String): RateLimitDecision =
            RateLimitDecision.allow(Long.MAX_VALUE)
    }
}

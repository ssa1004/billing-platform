...
interface SpringDataWebhookDeliveryRepository : JpaRepository<WebhookDeliveryJpaEntity, UUID> {

    /**
     * 워커 픽업 query — PENDING 이면서 next_attempt_at 도달한 것들을 SKIP LOCKED 로 잠금.
     *
     * 여러 워커 인스턴스가 동시에 호출해도 같은 row 를 두 번 잡지 않는다 (PG 의 SKIP LOCKED
     * 가 lock 못 잡은 row 는 그냥 결과에서 빼버림). H2 는 SKIP LOCKED 를 무시하지만 단일
     * 워커 dev 환경에선 문제 없음.
     *
     * `jakarta.persistence.lock.timeout = 0` → lock 못 잡으면 즉시 다음 row 시도.
     */
    ...
    fun claimPending(@Param("now") now: Instant, pageable: Pageable): List<WebhookDeliveryJpaEntity>

    fun findByEndpointIdOrderByCreatedAtDesc(
        endpointId: UUID,
        pageable: Pageable,
    ): List<WebhookDeliveryJpaEntity>

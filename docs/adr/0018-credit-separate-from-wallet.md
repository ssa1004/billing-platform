...
청구서 발행 직전 차감되는 *현금화 불가* (비-환전성) 잔액 풀이 필요합니다. 마케팅 프로모 /
신규 가입 free credit / SLA (서비스 수준 약속) 위반 보상 / 환불을 현금 대신 크레딧으로 적립
같은 케이스가 많습니다. 이 잔액은:

- 거래 입금 / 출금이 아니라 *발급* 됨 (운영자 / 마케팅 시스템 / CS)
- 만료 가능 (프로모 크레딧은 보통 30~90일)
- 환불 불가 (현금이 아님)
- 회계상 부채 인식 시점이 거래 잔액과 다름 (수익 인식 회계 처리는 별도)
- 청구서에 자동 적용되어 실제로 결제해야 할 금액을 줄임

## 결정

기존 {@code Wallet} 애그리거트와 합치지 않고 **별도 {@code Credit} 애그리거트** (한
트랜잭션으로 같이 관리되는 일관성 단위) 를 둡니다.
...
1. 만료 임박한 것 먼저 (그대로 두면 만료되어 사라질 잔액 손실을 최소화)
2. 만료가 같으면 발급 시점이 빠른 것 (FIFO, 먼저 들어온 게 먼저 나감)
...
- **Wallet 에 `walletType` 필드 추가** — 불변 조건이 서로 다르고 (Wallet 은
  입금/출금/블록까지, Credit 은 차감/만료/회수만), 회계 처리 분리도 깨져 거부.
- **Ledger entry 만으로 모델링** — Ledger 는 *기록* (한 번 적으면 수정 안 함) 용. 만료 /
  우선순위 / 회수 같은 상태 라이프사이클을 다루기에는 표현력이 부족.
...
- 만료 / 회수 같은 Credit 고유 라이프사이클을 Wallet 코드에 영향 없이 추가 가능
- 회계 처리도 Ledger 에 entry type 분리로 자연스럽게 표현
- (단점) customer 별 잔액 조회 시 두 테이블 join 필요 — 운영 화면용 read model 을 따로 만들면
  해결

## 참고

- 여러 tenant 를 한 시스템에 올린 SaaS 빌링에서는 Credit / Coupon / Discount 분리 패턴이
  흔함

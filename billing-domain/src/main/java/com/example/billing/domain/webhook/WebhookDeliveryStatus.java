 ...
 * <p><b>왜 4가지로 나뉘나</b>:
 * <ul>
 *   <li>{@code PENDING}: 큐에 들어가 worker 가 픽업 대기. {@code FOR UPDATE SKIP LOCKED} 로 잡음.</li>
 *   <li>{@code IN_FLIGHT}: worker 가 픽업해 HTTP POST 진행 중. 워커가 죽어도 복구되도록 timeout 설정 필요.</li>
 *   <li>{@code SUCCESS}: 2xx 응답 받음. 종착.</li>
 *   <li>{@code DEAD_LETTERED}: 재시도 다 썼거나 영구 실패 (4xx 등). 운영자 화면에서 보고 수동 replay 가능.</li>
 * </ul>
 */

...
ADR-0008 의 Resilience4j Circuit Breaker (서킷브레이커) 는 실패 누적 시 호출 차단으로 자원
보호. 그런데 CB OPEN 직전 — 즉 호출이 느려지는 단계가 위험합니다.
...
- **Semaphore**: 호출 스레드 자체의 동시성만 제한. 호출은 caller thread 에서 그대로 실행.
...
따라서 전파 차단의 본질적 효과는 ThreadPool 만 가능. 본 ADR 의 핵심 결정.
...
- 내부에서 `bulkhead.executeSupplier(...)` 로 별도 worker 에서 실행 + caller 는 짧은 timeout
  으로 future.get() 대기.
- caller 의 동기 시그니처는 유지하면서, worker 는 격리 풀에 있어 전파 차단 효과는 그대로.

핵심 장단점: caller thread 도 wait 하긴 함 (가상 스레드라 OS thread 는 안 잡힘). 그래도
bulkhead 가득 차면 즉시 fast-fail + worker 풀이 별도라 다른 도메인 endpoint 영향 없음 —
두 핵심 효과는 살아있음.

가상 스레드 (Java 21) 와 결합되니 caller wait 비용이 플랫폼 스레드 대비 자릿수가 작음.
...
는 외부 호출의 자원 보호용이라 Mock 호출에 의미 없음. `@ConditionalOnProperty` 로 prod
에서만 활성.

## 용어 풀이 (쉽게)

- **Bulkhead (격벽 격리)** — 배가 한 칸에 구멍 나도 안 가라앉게 칸막이로 나누듯, 외부 호출을 도메인별 전용 풀(PG용·webhook용)에 가둬 한 곳의 장애가 전체로 안 번지게 하는 것.
- **cascade failure (연쇄 장애)** — 한 곳(PG)이 느려져 그걸 호출하는 쪽까지 줄줄이 멈추는 도미노 장애. 느려진 PG 응답을 기다리느라 스레드가 다 묶이면 멀쩡한 지갑 조회까지 같이 멈춘다.
- **Semaphore vs ThreadPool Bulkhead** — Semaphore는 '동시에 몇 명까지'만 세고 호출은 부른 스레드가 직접 한다(느려지면 그 스레드도 같이 묶임). ThreadPool은 별도 일꾼에게 시키고 부른 쪽은 결과표만 받아, 일꾼이 묶여도 부른 쪽은 풀려난다 — 그래서 전파 차단엔 ThreadPool이 더 강하다.
- **fast-fail (빠른 실패)** — 풀이 꽉 차면 무작정 기다리지 않고 즉시 "지금 안 됨(503)"으로 떨어뜨려 자원을 지키는 것. 줄이 너무 길면 받지 않고 바로 돌려보내 가게가 마비되는 걸 막는 식.
- **Little's law (동시 호출 수 ≈ 처리량 × 평균 지연)** — 풀 크기를 추정하는 공식. 초당 10건 오고 한 건이 4초 걸리면 동시에 약 40건이 떠 있다는 계산으로, 그만큼 일꾼/대기열을 준비한다.
...
- **Servlet thread pool 자체를 도메인별로 분리** (Tomcat custom executor): 가능. Spring MVC 가
  `AsyncTaskExecutor` 로 도메인별 분기를 잘 지원하지만 외부 호출과 servlet 처리가 같은
...
- PG / webhook / audit-export 가 도메인별 격리 풀로 분리 — 한쪽 슬로우다운이 다른 쪽으로
  번지지 않음.
...
- (단점) Pool 산정이 운영 metric 없이 추정값 — 6개월 후 실측 latency / TPS 기반 재조정.

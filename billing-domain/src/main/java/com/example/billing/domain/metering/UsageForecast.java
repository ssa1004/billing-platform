...
/**
 * 한 customer × 한 BillingPeriod 의 *현재까지 사용량* 과 *월말 예상치* 를 묶은 값 객체.
 *
 * <p>외부에서 직접 만들지 않는다 — application service 가 read model 로 계산해 반환.
 * write side 가 없는 read-only 도메인 모델 (DTO 와 다른 점: 단위 / 통화 invariant 를 가짐).</p>
 *
 * <p>현재는 단순 *선형 외삽* (mtdQuantity / progressRatio) 으로 projectedQuantity 를 계산.
 * 정확도가 더 필요하면 7일 이동 평균 / 시계열 모델 (ARIMA / 회귀) 등으로 service 만 교체.</p>
 */

 ...
 * 구현 — {@code pg_advisory_xact_lock} 사용.
 *
 * <p>{@code _xact_} 변종이라 트랜잭션이 끝나면 자동으로 해제됩니다. 같은 트랜잭션에서 같은
 * 키를 두 번 lock 해도 멱등성 (반복 호출해도 결과 동일) 이 보장됩니다.</p>
 *
 * <p>{@code hashtext()} 로 문자열 키를 64비트 정수로 해시합니다. 이론적으로 충돌 가능하지만
 * 키 공간이 매우 클 때만 (1억 개 이상) 실용적 위험. billing 도메인의 키 (예:
 * settlement:cust:202605) 는 충돌 위험을 무시 가능.</p>
 */
@Component

...
- `billing-platform.yaml` — 빌드 시 생성되는 OpenAPI 3 문서. 외부 참조 / SDK codegen 의 단일 진실값.
  - 결제 / 환불 / Wallet / Credit (`/api/v1/payments`, `/api/v1/refunds`, `/api/v1/wallet`, `/api/v1/credits`)
  - Metering / Usage / 주문 (`/api/v1/usage`, `/api/v1/orders`)
  - Invoice (v1 + v2) / Settlement / 채권 (`/api/v1/invoices`, `/api/v2/invoices`, `/api/v1/settlement`)
